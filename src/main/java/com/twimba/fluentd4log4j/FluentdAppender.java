...
public class FluentdAppender extends AppenderSkeleton {
	...
	/**
	 * The fluentd tag prefix to be used <b>Default:</b> "" (empty string)
	 */
	...
	/**
	 * The fluentd server host to where to send the log messages. <b>Default:</b>
	 * "localhost"
	 */
	private String host = "localhost";
	/**
	 * The fluentd server port to where to send the log messages. <b>Default:</b>
	 * 24224
	 */
	...
	/**
	 * The socket buffer capacity to connect to the fluentd server <b>Default:</b>
	 * 1048576 (1Mb)
	 */
	private int bufferCapacity = 1024 * 1024; // 1M

	/**
	 * Switch from the default Exponential Delay reconnector to a constant delay
	 * reconnector <b>Default:</b> false (use the Exponential Delay reconnector)
	 */
	...
	public void activateOptions() {
		...
		try {
			...
			LogLog.warn("FluentdAppender connected to fluentd! (host=" + host + ", port=" + port + ",timeout=" + timeout
					+ ",bufferCapacity=" + bufferCapacity + ",tagPrefix=" + tagPrefix + ")");
		} catch (Exception e) {
			LogLog.warn("FluentdAppender NOT connected to fluentd! (host=" + host + ", port=" + port + ",timeout="
					+ timeout + ",bufferCapacity=" + bufferCapacity + ",tagPrefix=" + tagPrefix + ")");
			fluentLogger = null;
		}
	}
	...
	protected void append(LoggingEvent event) {
		if (fluentLogger == null) {
			...
			LogLog.debug(
					"FluentdAppender has no fluentLogger. Please check your configuration and/or logs for errors!");
			return;
		}
		...
		if (addHostname) {
			try {
				...
				LogLog.warn(
						"FluentdAppender is unable to get the current hostname. Please check your configuration and/or disable the addition of the hostname!");
			}
		}
		
		String tag = event.getLoggerName().startsWith(this.tag) ? event.getLoggerName() : this.tag; 
	...
	private String getTimestamp(LoggingEvent event) {
		...
		ZonedDateTime date = ZonedDateTime.ofInstant(Instant.ofEpochMilli(event.getTimeStamp()), ZoneOffset.of("-03:00"));
